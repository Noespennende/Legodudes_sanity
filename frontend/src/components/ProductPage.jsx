...
export default function ProductPage() {
    ...
    //handlesubmitt-funksjon for når en bruker sender en anmeldelse
    const handleSubmitt = async (e) => {
        e.preventDefault()
        if(rating=== 0) {
            setFormMessage("Du må sette en virdering.")
        }else {
            const result = await updateReview(product._id, reviewer, comment, rating)
            if(result == "Success"){
                setFormMessage("Din anmeldelse er regustrert!")
                setProduct(product.reviews.push({reviewer: reviewer, comment: comment, rating: rating}))
            } else {
            ...
            <main id="productpage">
                ...
                <article>
                    ...
                    <form>
                        ...
                        <p><button onClick={handleSubmitt()}>Send anmeldelse</button></p>
                    </form>
                    {
